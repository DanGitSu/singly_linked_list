...
void Forward_list<T>::append(const T& data)
{
    Node* curr = head_;
    if (curr != nullptr) // not empty
    {
        while (curr->next != nullptr){ // not at end
            curr = curr->next; // move to next
        }
        curr->next = new Node(data); // add new
    }
    else // if empty
    {
        head_ = new Node(data);
...
{
    if (other.head_ == nullptr) // check for empty
    {
        return;
    }
    else
    {
        ...
        head_ = new Node(otherNode->data); // new list
        otherNode = otherNode->next;
        size_++;
        while (otherNode != nullptr) // not at end
        {
            append(otherNode->data);
            otherNode = otherNode -> next;
        }
    }
}
...
void Forward_list<T>::merge(Forward_list& other)
{
    ...
    Node * curr = head_;
    Node * temp;
    if (curr || otherCurr != nullptr)
    {
        while(otherCurr->next != nullptr) // not empty
        {
            curr->data > otherCurr->data ? curr->next = otherCurr : otherCurr->next = curr;
